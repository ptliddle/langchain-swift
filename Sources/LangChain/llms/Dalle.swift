//
//  File.swift
//  
//
//  Created by 顾艳华 on 2023/8/22.
//

import Foundation
import NIOPosix
import AsyncHTTPClient
import OpenAIKit

public class Dalle: OpenAI {
    
    let size: DalleImage.Size
    public init(apiKey: String, baseUrl: String, size: DalleImage.Size, callbacks: [BaseCallbackHandler] = [], cache: BaseCache? = nil) {
        self.size = size
        super.init(apiKey: apiKey, baseUrl: baseUrl, callbacks: callbacks, cache: cache)
    }
    
    public override func _send(text: String, stops: [String] = []) async throws -> LLMResult {
        
        let eventLoopGroup = ThreadManager.thread
        let httpClient = HTTPClient(eventLoopGroupProvider: .shared(eventLoopGroup))
        defer {
            // it's important to shutdown the httpClient after all requests are done, even if one failed. See: https://github.com/swift-server/async-http-client
            try? httpClient.syncShutdown()
        }
        
        let openAIClient = try initiateChat(httpClient)
        
        let reps = try await openAIClient.images.create(prompt: text, size: dalleTo(size: size))
        return LLMResult(llm_output: reps.data.first!.url)
    }
}
